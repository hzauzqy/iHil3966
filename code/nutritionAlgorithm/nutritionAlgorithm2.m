function [newDietModel,pointsModel,roiFlux,pointsModelSln,menuChanges,detailedAnalysis] = nutritionAlgorithm2(model,rois,roisMinMax,options)
%function [pointsModel,rois,metsRoi,metsStoich] = nutritionAlgorithm2(model,rois,roisMinMax,options)
...
for i=1:length(rois)
    evalc('pointsModel= removeRxns(pointsModel, rois{i}, false, false)');
end
...
menuChanges=table([pointsModel.rxns(slnIndexes1);pointsModel.rxns(slnIndexes2)],pointsModelSln.v([slnIndexes1;slnIndexes2]),'VariableNames',{'FoodRxn', 'Flux'});
if strcmp(display,'on')
    menuChanges
end
